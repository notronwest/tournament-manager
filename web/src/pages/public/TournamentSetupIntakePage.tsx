...
// The token-scoped setup RPCs (get_setup_by_token / save_setup_by_token) are
// live on the backend but aren't in the generated Supabase types yet, so we
// call them through a thin typed wrapper rather than hand-editing the
// generated file. Regenerate types (supabase gen types) to drop this later.
type RpcCall = <T>(
  fn: string,
  args: Record<string, unknown>,
) => Promise<{ data: T | null; error: { message: string } | null }>;
// NB: bind — supabase.rpc relies on `this`, so an unbound reference would call
// it with `this === undefined` and blow up inside the client.
const rpc = supabase.rpc.bind(supabase) as unknown as RpcCall;

type SetupPayload = {
  id: string;
  status: string;
  answers: Partial<Form> | null;
  submitted_at: string | null;
  event_name: string | null;
  event_dates: string | null;
};

type EventContext = { event_name: string | null; event_dates: string | null };
...
export default function TournamentSetupIntakePage() {
  ...
  // A missing token is settled at first render (nothing to fetch); a present
  // token starts in the loading state until get_setup_by_token resolves.
  const [loading, setLoading] = useState(Boolean(token));
  const [loadError, setLoadError] = useState<string | null>(
    token ? null : "This setup link is missing its code.",
  );
  ...
  // Load the saved setup by token, prefilling the form with any answers on
  // file. A missing/invalid token short-circuits into an error panel.
  useEffect(() => {
    if (!token) return;
    let alive = true;
    rpc<SetupPayload>("get_setup_by_token", { p_token: token })
      .then(({ data, error }) => {
        if (!alive) return;
        if (error || !data) {
          setLoadError(
            "This setup link isn't valid or has expired. Check with the organizer for a new one.",
          );
          setLoading(false);
          return;
        }
        // The saved answers are already a Form-shaped object; merge over EMPTY
        // so fields added since the last save fall back to their defaults.
        setForm({ ...EMPTY, ...(data.answers ?? {}) });
        setEventContext({
          event_name: data.event_name,
          event_dates: data.event_dates,
        });
        if (data.status === "submitted" || data.status === "complete") {
  ...
  // Persist answers. p_submit=true finalizes (thank-you screen); false is a
  // "save & finish later" checkpoint that keeps the user in the wizard.
  const persist = async (finalize: boolean) => {
    setSaving(true);
    setSaveError(null);
    setSavedNote(null);
    const { data, error } = await rpc<{ ok: boolean; status: string }>(
      "save_setup_by_token",
      { p_token: token, p_answers: form, p_submit: finalize },
    );
    setSaving(false);
    if (error || !data?.ok) {
      setSaveError(
        error?.message ||
          "We couldn't save your setup just now. Please try again in a moment.",
      );
      return;
    }
    if (finalize) {
      ...
      setSavedNote(
        "Saved — you can close this and come back to the link anytime.",
      );
    }
  };
    ...
    return (
      <div style={pageWrapStyle}>
        <main style={{ ...contentColStyle(600), padding: "64px 24px 120px" }}>
          <div style={{ ...statusPanelStyle("danger"), padding: "20px 24px" }}>
            <div
              style={{
                fontSize: 18,
            ...
            >
              We couldn't open this setup form
            </div>
    ...
    <div style={pageWrapStyle}>
      <main style={{ ...contentColStyle(760), padding: "40px 20px 120px" }}>
        ...
        {eventContext?.event_name && (
          <div style={{ fontSize: 14, color: inkSoft, marginBottom: 8 }}>
            Setting up: <strong style={{ color: ink }}>{eventContext.event_name}</strong>
            {eventContext.event_dates ? ` · ${eventContext.event_dates}` : ""}
          </div>
        )}
        ...
        {/* Save error + transient save confirmation */}
        {saveError && (
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginTop: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
            <button
              style={{ ...ctaSecondaryStyle, opacity: saving ? 0.6 : 1 }}
            ...
            {step < STEPS.length - 1 ? (
              <button style={ctaPrimaryStyle} onClick={() => goto(step + 1)} disabled={saving}>
                Next: {STEPS[step + 1]} →
              </button>
            ) : (
              <button
                style={{ ...ctaPrimaryStyle, opacity: saving ? 0.6 : 1 }}
                onClick={() => persist(true)}
                disabled={saving}
              >
                {saving ? "Submitting…" : "Submit setup"}
              </button>
            )}
          </div>
        </div>
