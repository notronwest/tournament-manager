...
--
-- This CHANGES withdraw_self's return type (adds promoted_* columns), so a plain
-- `create or replace` errors with 42P13 ("cannot change return type of existing
-- function"). Drop it first, then recreate + re-grant below.
drop function if exists public.withdraw_self(uuid);
