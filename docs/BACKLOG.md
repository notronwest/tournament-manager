...
- 🚧 **Slice 6 (cleanup, deferred): Drop legacy columns + sync triggers** — `tournaments.entry_fee_cents`, `tournaments.additional_event_fee_cents`, the `20260526170001` triggers, the `TournamentFormPage` mirror-write + defensive fallback read. Pure cleanup now that nothing reads the legacy columns; safe to do anytime. Note `pricing.ts`'s `computeLineItems` keeps its `{entry_fee_cents, additional_event_fee_cents}` param shape (generic algorithm interface, fed by tier values).
