...
- 🚧 **Slice 5 (next up): Migrate remaining read sites** — `PendingPaymentsContext`, `TournamentsListPage`, `TournamentDetailPage`, `HomePage` switch from legacy fee columns to tier reads (e.g. show the active tier, or "from $X" for multi-tier).
