...
- 🚧 **Slice 4 (next up): Wizard.** Replace the single-fee pricing step in `TournamentFormPage` with the four-pattern picker mocked in `mockups/tournament-creation-flow.html` Step 3 (variants 4a–4d). Wizard writes tier rows directly; during the bridge period, *also* mirror tier 1 into the legacy columns so remaining read sites stay accurate.
- 🚧 **Slice 5: Migrate remaining read sites** — `PendingPaymentsContext`, `TournamentsListPage`, `TournamentDetailPage`, `HomePage` switch from legacy fee columns to tier reads.
- 🚧 **Slice 6: Drop legacy columns + sync triggers** — `tournaments.entry_fee_cents`, `tournaments.additional_event_fee_cents`, and the `20260526170001` triggers get removed in a follow-up migration once every read path is on tiers.
- 🚧 **Slice 7: Public status pill** — derive "Early Bird Registration Open" / "Registration Open" / "Late Registration Open" from `pricing_pattern` + active tier and render it on the public tournament page (the second surface of "one concept, two surfaces"). Custom tournaments render the organizer's literal tier name instead.
